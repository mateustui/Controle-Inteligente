...
f1 = open('entrada5.csv', 'w', newline='', encoding = 'utf-8')
w1 = csv.writer(f1)
f2 = open('saida5.csv', 'w', newline='', encoding = 'utf-8')
w2 = csv.writer(f2)
...
while True:
    ...
    w1.writerow([sensores[0] - env.xRef,sensores[1],sensores[2],sensores[3]])
    acao = funcao_controle_4(sensores)  # É ESSA A FUNÇÃO QUE VOCÊS DEVEM PROJETAR.
    #acao = model2.predict(sensores.reshape(1,4))
    w2.writerow([acao])
