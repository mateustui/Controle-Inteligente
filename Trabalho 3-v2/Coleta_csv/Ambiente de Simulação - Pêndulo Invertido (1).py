...
#with open('./teste.csv', 'w') as csvfile:
   # csv.write(csvfile, delimiter=',').writerow([])

def write_csv(a,b,c,d,e):
    with open('teste.csv', mode='a',  newline='') as csvfile:
        teste = csv.writer(csvfile, delimiter=';',quotechar='|', quoting=csv.QUOTE_MINIMAL)
        teste.writerow([a,b,c,d,e])   
...
def funcao_controle_1(sensores):
    ...
    write_csv(sensores[0]-env.xRef,sensores[1],sensores[2],sensores[3],acao)
    return acao
...
env = InvertedPendulum(randrange(-10, 10)/10)
